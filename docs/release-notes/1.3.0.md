...
- New `IconPacks` for MahApps: `PackIconMaterial`, `PackIconModern` and `PackIconFontAwesome` #2328
    + Usage:  
        ```
        ...
        ```
    + Icon names can be found in `PackIconMaterialKind`, `PackIconModernKind` or `PackIconFontAwesomeKind`
